...
const VacationsPage: React.FC<VacationsPageProps> = ({ records, setRecords, collaborators, holidays }) => {
  ...
  const handleDeleteBulk = () => {
    ...
    if (confirm(`Deseja realmente excluir os ${selectedIds.size} registros selecionados?`)) {
                  ...
                  >
                    ...
                    <td className="px-4 py-6">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] text-[#484F58] font-black uppercase tracking-widest">{collab?.unit}</span>
                      </div>
                    </td>
                    ...
                    {isAdmin && (
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <button onClick={() => handleDeleteSingle(record.id)} className="p-3 text-[#8B949E] hover:text-rose-500 hover:bg-rose-900/20 rounded-xl transition-all">
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </td>
                  ...
                  {!isInitialBalance && (
                    <div className="mt-10 flex items-start gap-4 bg-[#0D1117] p-6 rounded-3xl border border-[#30363D]">
                      ...
                      <p className="text-[10px] text-[#8B949E] font-bold uppercase tracking-widest leading-relaxed">
                        CÁLCULO AUTOMÁTICO BASEADO NA UNIDADE <strong>{formData.unit}</strong>.
                      </p>
                    </div>
                  )}
                </div>
